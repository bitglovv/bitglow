...
export function broadcastPresence(clients: Set<ClientMeta>) {
    ...
    for (const c of clients) {
        if (c.onlineVisible) onlineCount++;
    }
...
export function broadcastRoomPresence(clients: Set<ClientMeta>, roomId: string) {
    ...
    const activeInRoom = Array.from(clients).filter(
        (c) => c.socket.readyState === WebSocket.OPEN && c.rooms.has(roomId) && c.onlineVisible
    );
...
export async function handleMessage(
    ...
    switch (msg.type) {
        ...
        case "client:chat:send": {
            ...
            const previousUserMessageAt = userMessageTimestamps.get(meta.userId) || 0;
            ...
            userMessageTimestamps.set(meta.userId, now);
