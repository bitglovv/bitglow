...
async function postRoutes(fastify) {
    ...
    fastify.get("/posts/:id/comments", { preHandler: fastify.requireAuth, schema: schemas_1.idParamSchema }, async (req, reply) => {
        const userId = req.auth.id;
        const { id } = req.params;
        const comments = await db_1.db.getComments(id, userId);
        return { comments };
    });
    fastify.post("/comments/:id/like", {
        ...
        const userId = req.auth.id;
        const { id } = req.params;
        const result = await db_1.db.toggleCommentLike(userId, id);
    ...
    fastify.delete("/comments/:id", {
        ...
        const userId = req.auth.id;
        const { id } = req.params;
        const ok = await db_1.db.deleteComment(userId, id);
        if (!ok)
            return reply.code(403).send({ message: "Not authorized or comment not found" });
        return { ok: true };
    });
    fastify.put("/posts/:id", {
        ...
        try {
            ...
            const post = await db_1.db.getPostById(id, userId);
            if (!post) {
    ...
    fastify.get("/posts/:id", { preHandler: fastify.requireAuth, schema: schemas_1.idParamSchema }, async (req, reply) => {
        const userId = req.auth.id;
        const { id } = req.params;
