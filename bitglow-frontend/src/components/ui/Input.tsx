    ...
    ({ label, error, helperText, leftIcon, rightIcon, className = '', ...props }, ref) => {
        return (
            <div className="w-full space-y-2">
                ...
                <div className="relative group">
                    ...
                    <input
                        ref={ref}
                        className={`
              w-full rounded-2xl border border-[#102C26] bg-[#102C26]/15 px-4 py-4
            ...
            `}
                    ...
                    />
                    {rightIcon && (
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 flex items-center justify-center text-zinc-500">
                            {rightIcon}
                        </div>
                    )}
                </div>
                {error && <p className="ml-1 text-xs text-red-400">{error}</p>}
                {!error && helperText ? <p className="ml-1 text-xs text-zinc-400">{helperText}</p> : null}
            </div>
        );
    }
);
